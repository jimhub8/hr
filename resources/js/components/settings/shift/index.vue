            ...
            <v-flex sm12>
                <v-card style="padding: 20px 0;">
                    <el-breadcrumb separator-class="el-icon-arrow-right">
                        ...
                        <el-breadcrumb-item>Shift</el-breadcrumb-item>
                    </el-breadcrumb>
                </v-card>
            </v-flex>
            <v-flex sm12>
            ...
            <v-flex sm12>
                <v-card style="padding: 10px 0;">
                    <v-layout row>
                        <v-flex sm1 style="margin-left: 10px;">
                            <v-tooltip right>
                                <template v-slot:activator="{ on }">
                                    <v-btn icon v-on="on" slot="activator" class="mx-0" @click="getShifts">
                        ...
                        <v-flex sm2>
                            <h3 style="margin-left: 30px !important;margin-top: 10px;">Shift</h3>
                        </v-flex>
                        <v-flex offset-sm8 sm2>
                            <v-btn color="info" @click="openCreate" text>Create Shift</v-btn>
                        </v-flex>
                    </v-layout>
                </v-card>
            </v-flex>
            <v-flex sm12>
                <vue-good-table class="table-hover" :columns="columns" :rows="shifts" :search-options="{ enabled: true }" :pagination-options="{enabled: true,mode: 'pages'}" v-loading="isLoading" :sort-options="{enabled: true, initialSortBy: {field: 'id', type: 'asc'}}">
...
export default {
    ...
    methods: {
        openCreate() {
            eventBus.$emit("openCreateShift");
        },
        openEdit(data) {
            eventBus.$emit("openEditShift", data);
        },
        openShow(data) {
            eventBus.$emit("openShowShift", data);
        },
        ...
        deleteItem(item) {
            this.$store.dispatch("deleteItem", "shifts/" + item.id).then(response => {
                this.$message({
                    type: 'success',
