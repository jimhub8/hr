            ...
            <v-flex sm12>
                <v-card style="padding: 20px 0;">
                    <el-breadcrumb separator-class="el-icon-arrow-right">
                        ...
                        <el-breadcrumb-item>Complain</el-breadcrumb-item>
                    </el-breadcrumb>
                </v-card>
            </v-flex>
            <v-flex sm12>
            ...
            <v-flex sm12>
                <v-card style="padding: 10px 0;">
                    <v-layout row>
                        <v-flex sm1 style="margin-left: 10px;">
                            <v-tooltip right>
                                <template v-slot:activator="{ on }">
                                    <v-btn icon v-on="on" slot="activator" class="mx-0" @click="getComplains">
                        ...
                        <v-flex sm2>
                            <h3 style="margin-left: 30px !important;margin-top: 10px;">Complain</h3>
                        </v-flex>
                        <v-flex offset-sm8 sm2>
                            <v-btn color="info" @click="openCreate" text>Create Complain</v-btn>
                        </v-flex>
                    </v-layout>
                </v-card>
            </v-flex>
            <v-flex sm12>
                <v-pagination v-model="complains.current_page" :length="complains.last_page" total-visible="5" @input="next_page(complains.path, complains.current_page)" circle v-if="complains.last_page > 1"></v-pagination>
            </v-flex>
            <v-flex sm12>
                <vue-good-table class="table-hover" :columns="columns" :rows="complains.data" :search-options="{ enabled: true }" :pagination-options="{enabled: true,mode: 'pages'}" v-loading="isLoading" :sort-options="{enabled: true, initialSortBy: {field: 'id', type: 'asc'}}">
...
export default {
    ...
    data() {
        return {
            ...
            payload: {
                model: 'complains',
                update_list: 'updateComplainsList'
            },
            checkedRows: [],
            columns: [{
                    label: "Id#",
    ...
    methods: {
        openCreate() {
            eventBus.$emit("openCreateComplain");
        },
        openEdit(data) {
            eventBus.$emit("openEditComplain", data);
        },
        openShow(data) {
            eventBus.$emit("openShowComplain", data);
        },

        confirm_delete(item) {
            this.$confirm('This will permanently delete the file. Continue?', 'Complain', {
        ...
        deleteItem(item) {
            this.$store.dispatch("deleteItem", "complains/" + item.id).then(response => {
                this.$message({
                    type: 'success',
    ...
    computed: {
        complains() {
            return this.$store.getters.complains;
        },
    ...
    mounted() {
        // this.$store.dispatch('getComplains');
        eventBus.$emit("LoadingEvent");
        this.getComplains();
    },
    created() {
        eventBus.$on("complainEvent", data => {
            this.getComplains();
        });
        ...
        eventBus.$on("responseChooseEvent", data => {
            console.log(data);
            if (data == "Excel") {
                eventBus.$emit("openEditComplain");
            } else {
                eventBus.$emit("openCreateComplain");
            }
        });
