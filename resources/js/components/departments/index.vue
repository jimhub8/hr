            ...
            <v-flex sm12>
                <v-card style="padding: 10px 0;">
                    <v-layout row>
                        <v-flex sm1 style="margin-left: 10px;">
                            <v-tooltip right>
                                <template v-slot:activator="{ on }">
                                    <v-btn icon v-on="on" slot="activator" class="mx-0" @click="getDepartments">
                        ...
                        <v-flex sm2>
                            <h3 style="margin-left: 30px !important;margin-top: 10px;">Department</h3>
                        </v-flex>
                        <v-flex offset-sm8 sm2>
                            <v-btn color="info" @click="openCreate" text>Create Department</v-btn>
                        </v-flex>
                    </v-layout>
                </v-card>
            </v-flex>
            <v-flex sm12>
                <v-pagination v-model="departments.current_page" :length="departments.last_page" total-visible="5" @input="next_page(departments.path, departments.current_page)" circle v-if="departments.last_page > 1"></v-pagination>
            </v-flex>
            <v-flex sm12>
                <vue-good-table class="table-hover" :columns="columns" :rows="departments.data" :search-options="{ enabled: true }" :pagination-options="{enabled: true,mode: 'pages'}" v-loading="isLoading" :sort-options="{enabled: true, initialSortBy: {field: 'id', type: 'asc'}}">
...
export default {
    ...
    data() {
        return {
            ...
            payload: {
                model: 'departments',
                update_list: 'updateDepartmentsList'
            },
            checkedRows: [],
            columns: [{
                    label: "Id#",
    ...
    methods: {
        openCreate() {
            eventBus.$emit("openCreateDepartment");
        },
        openEdit(data) {
            eventBus.$emit("openEditDepartment", data);
        },
        openShow(data) {
            eventBus.$emit("openShowDepartment", data);
        },
        ...
        deleteItem(item) {
            this.$store.dispatch("deleteItem", "departments/" + item.id).then(response => {
                this.$message({
                    type: 'success',
    ...
    created() {
        eventBus.$on("departmentEvent", data => {
            this.getDepartments();
        });
        ...
        eventBus.$on("responseChooseEvent", data => {
            console.log(data);
            if (data == "Excel") {
                eventBus.$emit("openEditDepartment");
            } else {
                eventBus.$emit("openCreateDepartment");
            }
        });
    ...
    //     next(vm => {
    //       if (vm.user.can["view departments"]) {
    //         next();
    //       } else {
    //         next("/");
    //       }
